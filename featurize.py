...
def uppercase_word_count_feature(text, freq):
    words = text.split()
    return sum(1 for word in words if word.isupper())

def uppercase_ratio_feature(text, freq):
    words = text.split()
    return sum(1 for word in words if word.isupper()) / len(words) if words else 0

def digit_count_feature(text, freq):
    return sum(c.isdigit() for c in text)

def special_char_counts_feature(text, freq):
...
def freq_congrats_feature(text, freq):
    return float(freq['congratulations'])

def freq_hi_feature(text, freq):
    return float(freq['hi'])

def freq_premium_feature(text, freq):
    return float(freq['premium'])
...
def generate_feature_vector(text, freq):
    ...
    feature.append(word_count_feature(text, freq))
    feature.append(uppercase_word_count_feature(text, freq)) 
    feature.append(avg_word_length_feature(text, freq))
    feature.append(uppercase_ratio_feature(text, freq))
    feature.append(digit_count_feature(text, freq))
    feature.append(special_char_counts_feature(text, freq))
    ...
    feature.append(freq_congrats_feature(text, freq)) 
    feature.append(freq_hi_feature(text, freq))
